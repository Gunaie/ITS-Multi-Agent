...
class QueryService:
    ...
    async def generate_answer(self, user_question: str, retrival_context: List[Document]) -> str:
        ...
        if not retrival_context:
            # 即使没检索到，也应保持身份认知
            prompt = f"""
            你是联想智能技术助手，专注于联想产品（ThinkPad/小新/YOGA 笔记本、ThinkCentre/天启台式机、平板、显示器、服务器等）的售后与技术支持。
            对于用户的问题："{user_question}"，当前的知识库中暂时没有找到相关的解决方案。
            请礼貌地告知用户当前知识库未覆盖该问题，并根据你的通用知识尝试给出建议，但需说明这些建议并非来自官方知识库。
            """
        ...
        for index, doc in enumerate(retrival_context):
            ...
            # 使用更明确的格式，突出文件名
            formatted_context.append(f"### 参考文件: {file_name}\n内容: {clean_content}")
        
        context_str = "\n\n".join(formatted_context)

        # 3. 定义提示词
        prompt = f"""
        你是联想官方技术支持助手，是一位经验丰富的联想产品高级技术支持专家。
        请基于下方的【参考资料】回答【用户问题】。
         ...
         【回答要求】：
         1.  **身份认知**：你是"联想智能技术助手"，专注于联想产品（ThinkPad/小新/YOGA/ThinkCentre/天启/平板/显示器/服务器等）的售后与技术支持。
         2.  **基于事实**：严格基于【参考资料】的内容回答，严禁编造资料中未提及的信息。如果资料无法回答问题，请直接回答："当前的知识库中暂时没有找到该问题的解决方案。"
         3.  **标注来源**：(极其重要)
             - 在回答每一个具体知识点或步骤时，必须在其后紧跟引用来源的文件名。
             - **严禁使用 [文件1]、[资料1] 等模糊引用**。必须使用【参考资料】中明确标注的"参考文件"名称。
             - 格式为：`[真实文件名.md]`。
