...
6. **长期遗留**:公网部署 + HTTPS + 百度 AK Referer 白名单收紧(当前 `*`)、Ragas 评测集扩充、`context_precision 0.583` 检索精度优化(检索混入弱相关文档,可调 top_k/重排)、main-backend 镜像重建(knowledge_base.py 超时 120s 与 technical_agent.md 去工具名两处修复目前是 docker cp 热补,宿主机文件已是新值,下次重建自然带上)
